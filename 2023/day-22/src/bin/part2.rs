...
fn get_num_bricks_settled(mut bricks: Vec<Brick>) -> u32 {
    let mut settled_bricks = HashSet::new();
...
fn process(input: &str) -> u32 {
    ...
    let mut num_bricks_settled = 0;

    for brick in &bricks {
        let bricks_without_brick: Vec<_> =
            bricks.clone().into_iter().filter(|o| brick != o).collect();
        num_bricks_settled += get_num_bricks_settled(bricks_without_brick);
    }
